"""AWS Bedrock Nova + Pinecone RAG — Unified CLI entry point.
...
class _Config:
    def __init__(self):
        load_dotenv()
        from pinecone_rag.config import PineconeSettings, AWSNovaSettings, ChunkingSettings, OpenAISettings
        p = PineconeSettings()
        a = AWSNovaSettings()
        c = ChunkingSettings()
        o = OpenAISettings()
        self.aws_region = a.aws_region
        self.aws_nova_embedding_model = a.aws_nova_embedding_model
        self.aws_nova_embedding_dimension = a.aws_nova_embedding_dimension
        self.aws_nova_expected_dim = a.aws_nova_embedding_dimension
        self.aws_nova_video_max_bytes = a.aws_nova_video_max_bytes
        self.openai_api_key = o.openai_api_key
        self.openai_transcription_model = o.openai_transcription_model
        self.pinecone_api_key = p.pinecone_api_key
        self.pinecone_index = p.pinecone_index
        self.pinecone_index_aws_nova_1024 = p.pinecone_index_aws_nova_1024 or p.pinecone_index
...
def _ingest(namespace: str, data_dir: str):
    ...
    from pinecone_rag.embeddings.aws_nova_provider import AwsNovaProvider
    from pinecone_rag.pinecone_client import upsert_vectors

    config = _Config()
    provider = AwsNovaProvider(config)
    provider.validate()
    root = Path(data_dir)
    ...
    for f in collect_files(root / "audio", AUDIO_EXTENSIONS):
        for t in provider.build_audio_targets(f):
            all_targets.setdefault(t.index_name, []).append({"id": sha256_hash(f"aud:{f}:{t.metadata.get('text','')[:30]}"), "values": t.vector, "metadata": t.metadata})
    ...
    print(f"✅ AWS Nova ingestion complete — {sum(len(v) for v in all_targets.values())} vectors")
...
def main():
    parser = argparse.ArgumentParser(description="AWS Bedrock Nova + Pinecone RAG Pipeline")
